...
async function processInBackground(
  ...
  try {
    await Promise.race([
      overallTimeout,
      voiceProfileCore(supabase, jobId, profileId, reelIds, userId, provider, keys, modelId),
    ])
  } catch (error) {
...
async function voiceProfileCore(
  ...
  try {
    ...
    log('info', `Generating voice profile with ${provider}/${modelId}`, { jobId })

    const profile = await generate(prompt, provider, keys, modelId)
    ...
    await supabase.from('processing_jobs').update({
      ...
      output_data: { transcriptions_used: available.length, model: `${provider}/${modelId}` },
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Whisper (transcrição) é sempre OpenAI — openai_api_key obrigatória mesmo
    // quando o LLM de geração é Anthropic.
    ...
    const { data: job, error: jobError } = await supabase
      ...
      .insert({
        ...
        input_data: { profile_id, reel_ids, model_provider: provider, model_id },
    ...
    const backgroundTask = processInBackground(
      supabase, job.id, profile_id, reel_ids, user_id, provider, keys, model_id
    )
